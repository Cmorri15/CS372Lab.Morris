#pragma once
#include <iostream>
#include <algorithm>

template <typename T>
class Vector {
public:
    // Iterator class using index as backing store
    class iterator {
    private:
        Vector<T>* vec;
        int index;
        
        friend class Vector<T>;
        
        iterator(Vector<T>* v, int idx) : vec(v), index(idx) {}
        
    public:
        iterator() : vec(nullptr), index(0) {}
        
        // Dereference operator
        T& operator*() {
            if (index < 0 || index >= vec->length) {
                std::cerr << "Iterator: index out of bounds\n";
                exit(1);
            }
            return vec->arr[index];
        }
        
        // Increment operator (prefix)
        iterator& operator++() {
            index++;
            return *this;
        }
        
        // Increment operator (postfix)
        iterator operator++(int) {
            iterator temp = *this;
            index++;
            return temp;
        }
        
        // Equality operator
        bool operator==(const iterator& other) const {
            return (vec == other.vec) && (index == other.index);
        }
        
        // Inequality operator
        bool operator!=(const iterator& other) const {
            return !(*this == other);
        }
    };
    
    // Const iterator class
    class const_iterator {
    private:
        const Vector<T>* vec;
        int index;
        
        friend class Vector<T>;
        
        const_iterator(const Vector<T>* v, int idx) : vec(v), index(idx) {}
        
    public:
        const_iterator() : vec(nullptr), index(0) {}
        
        // Dereference operator (const)
        const T& operator*() const {
            if (index < 0 || index >= vec->length) {
                std::cerr << "Const iterator: index out of bounds\n";
                exit(1);
            }
            return vec->arr[index];
        }
        
        // Increment operator (prefix)
        const_iterator& operator++() {
            index++;
            return *this;
        }
        
        // Increment operator (postfix)
        const_iterator operator++(int) {
            const_iterator temp = *this;
            index++;
            return temp;
        }
        
        // Equality operator
        bool operator==(const const_iterator& other) const {
            return (vec == other.vec) && (index == other.index);
        }
        
        // Inequality operator
        bool operator!=(const const_iterator& other) const {
            return !(*this == other);
        }
    };
    
    // Vector constructors and methods
    Vector() {
        arr = new T[1];
        vCapacity = 1;
        length = 0;
    }
    
    virtual ~Vector() {
        delete[] arr;
        arr = nullptr;
    }
    
    Vector(const Vector& obj) {
        this->arr = new T[obj.vCapacity];
        this->vCapacity = obj.vCapacity;
        this->length = obj.length;
        for (int i = 0; i < obj.length; i++) {
            this->arr[i] = obj.arr[i];
        }
    }
    
    Vector& operator=(const Vector& source) {
        if (this == &source) {
            return *this;
        }
        delete[] arr;
        arr = new T[source.vCapacity];
        vCapacity = source.vCapacity;
        length = source.length;
        for (int i = 0; i < source.length; i++) {
            arr[i] = source.arr[i];
        }
        return *this;
    }
    
    void push_back(T data) {
        if (length == vCapacity) {
            expand(2 * vCapacity);
        }
        arr[length] = data;
        length++;
    }
    
    void put(T data, int index) {
        if (index == vCapacity) {
            push_back(data);
        }
        else {
            arr[index] = data;
        }
    }
    
    T& at(int index) {
        if (index < 0 || index >= length) {
            std::cerr << "Vector: index out of bounds on access\n";
            exit(1);
        }
        return arr[index];
    }
    
    T& operator[](int i) {
        if ((i < 0) || (i >= length)) {
            std::cerr << std::endl << "Vector index out of bounds\n";
            exit(1);
        }
        return at(i);
    }
    
    int size() const {
        return length;
    }
    
    int capacity() const {
        return vCapacity;
    }
    
    void traverse() {
        for (int i = 0; i < length; i++) {
            std::cout << arr[i] << std::endl;
        }
        std::cout << std::endl;
    }
    
    bool operator==(const Vector& other) const {
        if (other.size() != length) {
            return false;
        }
        for (int i = 0; i < length; i++) {
            if (arr[i] != other.arr[i]) {
                return false;
            }
        }
        return true;
    }
    
    // Iterator methods
    iterator begin() {
        return iterator(this, 0);
    }
    
    iterator end() {
        return iterator(this, length);
    }
    
    const_iterator cbegin() const {
        return const_iterator(this, 0);
    }
    
    const_iterator cend() const {
        return const_iterator(this, length);
    }

protected:
    virtual void expand(int newCapacity) {
        if (newCapacity > vCapacity) {
            T *temp = new T[newCapacity];
            for (int i = 0; i < length; i++) {
                temp[i] = arr[i];
            }
            delete[] arr;
            vCapacity = newCapacity;
            arr = temp;
        }
        else {
            std::cerr << "vector::expand: new capacity is less than or equal to current\n";
        }
    }
    
    T *arr;
    int vCapacity;
    int length;
};